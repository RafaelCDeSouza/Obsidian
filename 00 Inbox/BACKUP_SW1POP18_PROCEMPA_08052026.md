```kotlin
SW1POP18_PROCEMPA# show running-config
aaa authentication-order [ local ]
aaa user admin
 password $1$QxoeSqEu$t3ImF1E8sPg0ohR5WvY1e/
 group admin
!
aaa user zandona
 password $1$CP4FKCf5$du5mwfCY4xywAkUVvj4Or.
 group admin
!
mpls ldp
 lsr-id loopback-0
  interface l3-L3_VLAN1118_MPLS
  !
  interface l3-L3_VLAN1218_MPLS
  !
  neighbor targeted 10.244.188.2
  !
  neighbor targeted 10.244.189.0
  !
 !
!
mpls l2vpn
 vpls-group POP18_PROCEMPA
  vpn VSI_181_POP0-2_POP18_CEIC_OLT1
   vfi
    pw-type ethernet
    neighbor 10.244.188.2
     pw-id 181
    !
    neighbor 10.244.189.0
     pw-id 181
    !
   !
   bridge-domain
    dot1q 181
    bridge-mtu 9000
    access-interface gigabit-ethernet-1/1/2
    !
   !
  !
  vpn VSI_3108_POP00_POP02_TR_POA_SEG
   vfi
    pw-type ethernet
    neighbor 10.244.189.0
     pw-id 3108
    !
   !
   bridge-domain
    bridge-mtu 9000
    mac-limit 1500
    access-interface gigabit-ethernet-1/1/1
    !
    access-interface gigabit-ethernet-1/1/4
    !
    access-interface gigabit-ethernet-1/1/5
    !
   !
  !
 !
!
snmp system location -30.158638,-51.169596
snmp traps config-commit
snmp traps cpu-core
snmp traps cpu-load
snmp traps link-status
snmp traps login-success
snmp agent enabled
snmp agent version v2c
snmp agent version v3
snmp community CyberWeb
 sec-name CyberWeb
!
snmp community public
 sec-name public
!
snmp notify std_v1_trap
 tag std_v1_trap
!
snmp notify std_v2_inform
 tag std_v2_inform
 type inform
!
snmp notify std_v2_trap
 tag std_v2_trap
!
snmp notify std_v3_inform
 tag std_v3_inform
 type inform
!
snmp notify std_v3_trap
 tag std_v3_trap
!
snmp vacm group CyberWeb
 member CyberWeb
  sec-model [ v2c ]
 !
 access "" v2c no-auth-no-priv
  read-view root
  notify-view root
 !
!
snmp vacm group public
 member public
  sec-model [ v2c ]
 !
 access "" v2c no-auth-no-priv
  read-view root
  write-view root
  notify-view root
 !
!
snmp vacm view root
 subtree 1.3
  included
 !
!
interface gigabit-ethernet 1/1/1
 description SW1POP18_P01_SWITCH_CEIC_VLAN3108
 no shutdown
 negotiation
 duplex full
 speed 100M
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix auto
 mtu 12266
!
interface gigabit-ethernet 1/1/2
 description SW1POP18_P02_CEIC_INTERNET_VLAN181
 no shutdown
 negotiation
 duplex full
 speed 100M
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix auto
 mtu 12266
!
interface gigabit-ethernet 1/1/3
 shutdown
 negotiation
 duplex full
 speed 100M
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix auto
 mtu 12266
!
interface gigabit-ethernet 1/1/4
 description MKT_PROCEMPA_LAB_P4
 no shutdown
 negotiation
 duplex full
 speed 100M
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix auto
 mtu 12266
!
interface gigabit-ethernet 1/1/5
 description SW1POP18_P05_SWITCH_DGT_VLAN3108
 no shutdown
 negotiation
 duplex full
 speed 1G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface gigabit-ethernet 1/1/6
 shutdown
 negotiation
 duplex full
 speed 1G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface gigabit-ethernet 1/1/7
 shutdown
 negotiation
 duplex full
 speed 1G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface gigabit-ethernet 1/1/8
 shutdown
 negotiation
 duplex full
 speed 1G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface ten-gigabit-ethernet 1/1/1
 shutdown
 no negotiation
 duplex full
 speed 10G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface ten-gigabit-ethernet 1/1/2
 shutdown
 no negotiation
 duplex full
 speed 10G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface ten-gigabit-ethernet 1/1/3
 shutdown
 no negotiation
 duplex full
 speed 10G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface ten-gigabit-ethernet 1/1/4
 description PTP_SW1POP13_PX04_SW1POP18_PX04
 no shutdown
 no negotiation
 duplex full
 speed 10G
 advertising-abilities 10Mfull 100Mfull 1Gfull
 mdix normal
 mtu 12266
!
interface l3 L3_VLAN1118_MPLS
 lower-layer-if vlan 1118
 ipv4 address 10.244.118.2/30
!
interface l3 L3_VLAN1218_MPLS
 lower-layer-if vlan 1218
 ipv4 address 10.244.118.6/30
!
interface mgmt 1/1/1
 ipv4 address 192.168.0.25/24
!
interface loopback 0
 ipv4 address 10.244.188.18/32
!
clock timezone BRA -3
hostname SW1POP18_PROCEMPA
telnet-server enabled
oam
 efm
  interface gigabit-ethernet-1/1/1
   mode passive
  !
  interface gigabit-ethernet-1/1/2
   mode passive
  !
  interface gigabit-ethernet-1/1/3
   mode passive
  !
  interface gigabit-ethernet-1/1/4
   mode passive
  !
  interface gigabit-ethernet-1/1/5
   mode passive
  !
  interface gigabit-ethernet-1/1/6
   mode passive
  !
  interface gigabit-ethernet-1/1/7
   mode passive
  !
  interface gigabit-ethernet-1/1/8
   mode passive
  !
  interface ten-gigabit-ethernet-1/1/1
   mode passive
  !
  interface ten-gigabit-ethernet-1/1/2
   mode passive
  !
  interface ten-gigabit-ethernet-1/1/3
   mode passive
  !
  interface ten-gigabit-ethernet-1/1/4
   mode passive
  !
 !
!
loopback-detection
 destination-address alternative
!
mac-address-table
 aging-time 600
!
layer2-control-protocol
 tunnel-mac datacom
 tunnel-priority 7
!
vrf global
!
vrf mgmt
!
router ospf 1 vrf global
 router-id 10.244.188.18
 redistribute static
 !
 area 0.0.0.0
  interface l3-L3_VLAN1118_MPLS
   network-type point-to-point
  !
  interface l3-L3_VLAN1218_MPLS
   network-type point-to-point
  !
  interface loopback-0
  !
 !
!
dot1q
 vlan 322
  name VLAN322_TRANSPORTE_INTERNO_CEIC
  interface gigabit-ethernet-1/1/2
  !
 !
 vlan 1118
  name VLAN1118_SW1POP18_SW1POPXX_MPLS
  interface ten-gigabit-ethernet-1/1/1
  !
  interface ten-gigabit-ethernet-1/1/4
  !
 !
 vlan 1218
  name VLAN1218_SW1POP18_SW1POPXX_MPLS
  interface ten-gigabit-ethernet-1/1/2
  !
 !
!
license mpls enabled
license mpls key ***
remote-devices
 interface gigabit-ethernet-1/1/1
 !
 interface gigabit-ethernet-1/1/2
 !
 interface gigabit-ethernet-1/1/3
 !
 interface gigabit-ethernet-1/1/4
 !
 interface gigabit-ethernet-1/1/5
 !
 interface gigabit-ethernet-1/1/6
 !
 interface gigabit-ethernet-1/1/7
 !
 interface gigabit-ethernet-1/1/8
 !
 interface ten-gigabit-ethernet-1/1/1
 !
 interface ten-gigabit-ethernet-1/1/2
 !
 interface ten-gigabit-ethernet-1/1/3
 !
 interface ten-gigabit-ethernet-1/1/4
 !
!
```
